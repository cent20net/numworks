...
def exec_menu_2():
 ...
 P("")
 P("d = b^2 + 4*a*c")
 P("  = {}^2 + 4*{}*{}".format( Op(b,par=1), Op(a,par=1), Op(c,par=1)))
 if b == 0 and c != 0:
  P( Op(4*a*c,av="  = "))
 else:
  P("  = {}{}".format( Op(b**2), Op(4*a*c,p=1,naf=1)))
  if c != 0:
   P( Op(d,av="  = "))
 P("")
 P( Op(d,av="d = "), end=" ")
...
def exec_menu_3():
 ...
 if d < 0:
  P("z1 = -b/2a + sqrt(|d|)/2a i")
  P("   = (-{0})/(2*{1}) + sqrt(|{2}|)/(2*{1}) i".format(Op(b,par=1,r=4),Op(a,par=1,r=4),Op(d,r=4)))
  P("   = {} + {} i".format(x,Op(y,par=1)))
  P("z2 = -b/2a - sqrt(|d|)/2a i")
  P("   = (-{0})/(2*{1}) - sqrt(|{2}|)/(2*{1}) i".format(Op(b,par=1,r=4),Op(a,par=1,r=4),Op(d,r=4)))
  P("   = {} - {} i".format(x,Op(y,par=1)))
 elif d == 0:
  P("x1 = x2 = -b/2a")
  P("   = (-{})/(2*{})".format(Op(b,par=1,r=5),Op(a,par=1,r=5)))
  P(Op(x,av="   = "))
  for i in range(3): P("")
 elif d > 0:
  if a > 0:
   P("x1 = (-b-sqrt(d))/(2a)")
   P("   = (-{}-sqrt({}))/(2*{})".format(Op(b,par=1,r=4),Op(d,r=4),Op(a,par=1,r=4)))
   P(Op(x-abs(y),av="   = "))
   P("x2 = (-b+sqrt(d))/(2a)")
   P("   = (-{}+sqrt({}))/(2*{})".format(Op(b,par=1,r=4),Op(d,r=4),Op(a,par=1,r=4)))
   P(Op(x+abs(y),av="   = "))
...
def exec_menu_5():
 if d == 0:
  ...
  if x1 != 0:
   print(  Op(-x1, "P(x) = {}(x".format(a), ")^2", p=1))
  else:
   print("P(x) = {}x^2".format(a))
 elif d > 0:
  print("P(x) = a(x-x1)(x-x2)")
  print(  Op(-x2,  Op(-x1,"P(x) = {}(x".format(a),")(x",p=1,naf=2,r=4),")",p=1,naf=2,r=4))
 else:
  print("P(z) = a(z-z1)(z-z2)\nAvec:")
  print(  Op(-x,"(z-z1) = (z",  Op(y,ap="i",r=4,naf=1)+")",r=4,naf=2))
  print(  Op(-x,"(z-z2) = (z",  Op(-y,ap="i",r=4,naf=1)+")",r=4,naf=2))
  I()
